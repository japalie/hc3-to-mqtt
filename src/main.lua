...
function QuickApp:dispatchFibaroEventToMqtt(event)
    ...
    if (device) then
        ...
        if (event.type == "DevicePropertyUpdatedEvent" and device.bridgeType ~= "scene") then
...
--            self:warning(json.encode(event.data))
--            self:warning(json.encode(device.bridgeType))
--            self:warning(json.encode(event))
            for i, j in ipairs(self.mqttConventions) do
                j:onPropertyUpdated(device, event)
            end
        ...
        elseif(device.bridgeType == "scene") then
            -- do nothing
        else
